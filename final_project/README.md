...
## Methodology
### Scheduling Policy 1: Minimum Average Response/Waiting Time
In this case, the optimal solution has an minimum average response/waiting time. 
This can be achieved by SJF policy. 

### Scheduling Policy 2: Minimum Execution Time
In this case, the optimal solution has an minimum average execution time. 
...
#### Greedy Choice
Best fit spaces. If it does not exist, then grow the rectangle that has sufficient resources and has the minimum start point.
...
* Expected results: (average block number on one SM)
...
#### Optimal Scheduling Policy 1
* Kernel launching order: {k1, k2, k3}
* Expected results: (average block number on one SM) The same as naive policy.

#### Optimal Scheduling Policy 2
...
| Opt 1   | 60                  | (20+40+60)/3=40            | (0+20+20)/3=13.33         |
| Opt 2   | 40                  | (20+40)/3=20               | (0+0+20)/3=6.67           |
...
* Expected results: (average block number on one SM) At t = 0, k4, k3, and k2 start running simultaneously. k1 cannot start until k3 finishes execution at t = 12. </br>
...
| Opt 1   | 32                  | (10+12+16+32)/4=18.5       | (0+0+0+16)/4=4            |
| Opt 2   | 28                  | (20+16+10+28)/4=18.5       | (0+0+0+16)/4=4            |
...
#### Naive Scheduling Policy (FIFO)
* Kernel launching order: {k1, k2, k3}
* Expected results: (average block number average on one SM)
...
| Opt 1   | 70                  | (20+30+70)/3=40            | (0+0+20)/3=6.67           |
| Opt 2   | 50                  | (50+30+50)/3=43.33         | (0+0+30)/3=10             |

### Scenario 4: generalized kernels
